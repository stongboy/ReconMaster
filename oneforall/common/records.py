...
class Connection(object):
    ...
    def query(self, query, fetchall=False, **params):
        ...
        # Execute the given query (SQLAlchemy 2.0: params 必须作为 dict 传入)
        cursor = self._conn.execute(text(query), params or {})

        # Python 3.13 sqlite3 会自动关闭 cursor，必须立即 fetch 全部行。
        if cursor.returns_rows:
            keys = list(cursor.keys())
            rows = [tuple(row) for row in cursor]
            row_gen = (Record(keys, row) for row in rows)
        else:
            # DML (INSERT/UPDATE/DELETE) — SQLAlchemy 2.0 requires explicit commit
            self._conn.commit()
            row_gen = iter([])
